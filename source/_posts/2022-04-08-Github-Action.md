...
网上寻找解决办法的过程中，发现了一个Jekyll博客的优化插件库 [jeffreytse/jekyll-spaceship](https://github.com/jeffreytse/jekyll-spaceship)，可以通过Markdown添加流程图、音乐，可以实现Html代码与Markdown同时使用、并优化了表格系统等等，如下图：
...
具体功能可以去作者库中看一看详细信息（[jeffreytse/jekyll-spaceship](https://github.com/jeffreytse/jekyll-spaceship)）。
...
1. 通过以下指令，在本地博客目录下建立一个新的 branch，命名为 gh-pages
...
2. 在自己的博客库根目录下添加文件 --> .github/workflows/build-jekyll.yml
3. build-jekyll.yml中添加如下
	```
	...
	注：可以将`branches:`下`master`改为自己的推送branch名，这里标注为默认值`master`。

3. 使用`master` branch再进行新一轮push后，如果正确配置将自动将master中的网站自动转换为静态网页代码添加到刚刚在第一步中建立的`gh-pages`branch中。

	```
	git push origin master
	```

	如何发生推送错误，可以在博客仓库的Action选项栏中观察报错原因中观察错误原因。

	![Action选项栏](/assets/20220408/2022-04-09-13-44-01.png) 

4. 进入仓库设置页面（Pages栏），将默认页面改为gh-pages即可。

	![更改默认生成页面的branch](/assets/20220408/2022-04-09-13-37-53.png)
