...
在平均值家族里，除了算术平均数和几何平均数，还有一个很常见的平均数：调和平均数。
...
它的直观含义是：先取倒数，再做算术平均，最后再取倒数。所以它对“小的数”更敏感。比如速度平均里，如果一半路程速度很慢，整体平均速度会被慢的那段明显拉低，这时调和平均数比算术平均数更合适。

对 $n$ 个正数 $x_1,x_2,\dots,x_n$，调和平均数定义为
...
也就是说，$H$ 的倒数等于这些数倒数的算术平均数。

调和平均数和几何平均数、算术平均数之间有一个基本链条：
...
右边的 $G\le A$ 就是 AM-GM。左边的 $H\le G$ 可以直接对倒数使用 AM-GM。

因为 $x_i>0$，所以 $\frac{1}{x_i}$ 也是正数。对
...
\frac{\frac{1}{x_1}+\frac{1}{x_2}+\cdots+\frac{1}{x_n}}{n}
\ge
\sqrt[n]{\frac{1}{x_1x_2\cdots x_n}}.
$$

左边是 $\frac{1}{H}$，右边是 $\frac{1}{G}$，所以

$$
\frac{1}{H}\ge \frac{1}{G}.
$$

由于 $H$ 和 $G$ 都是正数，两边取倒数时不等号方向反过来：
...
当 $x_i\ge 0$ 时，这说明算术平均数不超过平方平均数。把上一节的调和平均数也加进来，就得到常见的平均值链条：
